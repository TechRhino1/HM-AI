...
**pytest 2973 passed / 0 failed / 20 deselected** — green, not tolerated. Parse `--junit-xml`: the
...
* **A refused order is answered with HTTP 200** — decide from the body's `status`, never `res.ok`.
  Broker sends `reason`; server validation sends `error` with 400.
...
* **Truncating values cannot shrink a payload** (63 KB → 94%); drop *fields*: 33%.
...
* **A mutation that kills nothing may be unreachable** — an inner `except` swallowed it first. Never
  count a mutation as evidence until it turns something red. **Order mutations so they don't shadow.**
...
`…\binaries\python\versions\3.13.12\python.exe`; binds **127.0.0.1 only**.
