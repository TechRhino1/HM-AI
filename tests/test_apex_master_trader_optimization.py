...
class TestApexMasterTraderOptimization(unittest.TestCase):
    ...
    def test_decision_engine_quality_gate_thresholds_and_strict_discount_premium(self):
        ...
        gate_fx = self.decision_engine._apply_quality_gate(
            ...
            ev=10.0, final_win_p=0.48, spread=1.0, premium_discount_valid=True,
            account_balance=10000.0, tentative_bias="BUY", calibrated_win_p=0.48, risk_dist=0.0020, planned_risk_dollars=50.0
        )
        self.assertFalse(gate_fx.checks["AI Multi-Score Gate"], "Forex min_score=73.0 must fail score=62.0")
        self.assertFalse(gate_fx.checks["Calibrated Win Prob >= 50%"], "Forex required_win_p must fail win_p=0.48")
