...
def test_position_monitor_constants():
    """Stop-ratchet thresholds now live in jarvis.execution.exit_policy.

    PositionMonitorEngine must NOT re-declare them, otherwise live and backtest
    arithmetic drift apart again. Only the partial-TP fallbacks remain local.
    """
    ...
    # The whole point of the refactor: a late, adaptive breakeven — not 1R.
    assert DEFAULT_BE_TRIGGER_R == 2.0
    assert DEFAULT_FAST_CASH_R == 0.75
    assert DEFAULT_RUNNER_TRAIL_ATR == 1.2
