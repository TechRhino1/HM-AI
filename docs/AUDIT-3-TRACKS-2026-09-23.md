...
| **`max_wait` is implemented but not wired into `MacroAnalyst`** (§O) | The engine capability exists and is tested (`tests/test_news_bounded_wait.py`, 13 tests; `max_wait=None` is byte-for-byte the old behaviour). Wiring `max_wait≈1.5` into `macro_analyst.py:67` would stop the 2.0s cluster budget from replacing the analyst with a score-50 fallback. **Deliberately left as a call-site decision**, because it changes the inputs of a live trading system — and because after the §O provenance fix the practical effect is now *small and in the wrong direction*: the analyst would score ~65 (synthetic news excluded) instead of 50 (timeout fallback), i.e. **+2.5 on `ai_score` and more trades through the gate**, which §J says is the losing direction. |
