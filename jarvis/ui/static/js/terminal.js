...
(function () {
    ...
    let _lastPendingSnapshot = "";
    function renderPendingOrdersDOM(pending) {
        const tbody = el.pendingTbody || document.getElementById("pending-tbody");
        const countBadge = el.pendingCount || document.getElementById("pending-count");
        if (!tbody) return;
        if (countBadge) countBadge.textContent = `${pending.length}`;

        if (!Array.isArray(pending) || pending.length === 0) {
            tbody.innerHTML = '<tr><td colspan="9" style="text-align:center; color:var(--text-dim); padding:16px;">No Active Pending Orders</td></tr>';
            _lastPendingSnapshot = "";
            return;
        }

        const snapshot = JSON.stringify(pending);
        if (snapshot === _lastPendingSnapshot) return;
        _lastPendingSnapshot = snapshot;

        tbody.innerHTML = pending.map(p => {
            const orderType = p.type || "LIMIT";
            const isBuy = orderType.includes("BUY");
            const priceVal = p.price_open || p.price || 0;
    ...
    async function fetchPendingOrders() {
        try {
            ...
            if (Array.isArray(pending)) {
                ...
                renderPendingOrdersDOM(pending);
            }
        } catch (err) {
    ...
    window.closePosition = async function (ticket) {
        ...
        try {
            ...
            const data = await res.json();
            if (data.status === "CLOSED" || data.status === "PARTIALLY_CLOSED") {
                fetchTelemetry();
            } else {
    ...
    window.cancelPendingOrder = async function (ticket) {
        if (!confirm(`Confirm cancel pending order #${ticket}?`)) return;
        try {
            const res = await fetch("/api/action/cancel_pending_order", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ ticket })
            });
            ...
            if (data.status === "CANCELLED") {
                ...
                alert(`Cancel failed: ${data.reason || 'Unknown error'}`);
            }
        } catch (err) {
    ...
    window.executeManualTrade = async function (action) {
        ...
        try {
            ...
            if (data.status === "FILLED" || data.status === "PLACED") {
                alert(`Order ${data.status}: Ticket #${data.ticket} ${action} ${lots} ${sym} @ ${data.price}`);
                fetchTelemetry();
    ...
    window.switchDockTab = function (tabName) {
        ...
        const tabs = ["positions", "pending", "history", "analytics", "logs"];
