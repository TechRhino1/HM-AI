...
class InstitutionalVolumeOrderFlowEngine:
    ...
    def analyze_order_flow(self, df: pd.DataFrame) -> dict:
        ...
        # Resolve the volume column defensively. Different providers name it
        # 'volume', 'tick_volume' or 'real_volume', and some (e.g. synthetic or
        # options data) omit it entirely. Previously a missing column raised
        # KeyError and aborted the whole decision pipeline.
        vol_col = None
        ...
        if vol_col is None:
            # Without volume we cannot compute order flow. Degrade gracefully to a
            # neutral signal rather than crashing the caller.
            return {
                'signal': 'NEUTRAL',
                'strength': 0.0,
                'institutional_activity': False,
                ...
                'absorption_trap': None,
                'delta_score': 0.0,
                'note': 'no volume column; order flow unavailable'
            }

        # Fast numpy slice extraction
        vol_arr = df[vol_col].values[-self.volume_ma_period:].astype(float)
        close_arr = df['close'].values[-self.volume_ma_period:].astype(float)
        open_arr = df['open'].values[-self.volume_ma_period:].astype(float)
        # Note: `in df` on a DataFrame tests COLUMNS, which is the intended check
        # here, but we use .columns explicitly for clarity and to avoid the
        # pandas ambiguity warning on newer versions.
        high_arr = df['high'].values[-self.volume_ma_period:].astype(float) if 'high' in df.columns else close_arr
        low_arr = df['low'].values[-self.volume_ma_period:].astype(float) if 'low' in df.columns else open_arr
