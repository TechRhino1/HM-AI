...
class OnlineMLPredictor:
    ...
    def __init__(
        self,
        model_file: str = "jarvis_online_ml_weights.json",
        learning_rate: float = 0.05,
        l2_reg: float = 0.01,
        brier_drift_threshold: float = None,
        drift_shrink_factor: float = 0.92,
    ):
        # Resolve runtime settings from the central config so ML parameters are no
        # longer hardcoded here (previously MLSettings was defined but never read).
        try:
            from jarvis.config.settings import SETTINGS as _S
            _ml = _S.ml
            self.brier_drift_threshold = float(
                brier_drift_threshold if brier_drift_threshold is not None
                else _ml.brier_score_drift_threshold
            )
            self.learning_rate = float(_ml.sgd_learning_rate)
            self.l2_reg = float(_ml.sgd_l2_regularization)
        except Exception:
            self.brier_drift_threshold = float(
                0.28 if brier_drift_threshold is None else brier_drift_threshold
            )
            self.learning_rate = learning_rate
            self.l2_reg = l2_reg

        # Drift handling must be gradual and non-compounding.
        self.brier_drift_hysteresis = 0.07   # only act well above a coin-flip Brier
        self.brier_window_min = 30           # require a real sample before acting
        self.drift_cooldown_steps = 100      # never fire twice in quick succession
        self.drift_shrink_factor = float(drift_shrink_factor)

        self.model_file = self._resolve_model_path(model_file)
        ...
        self._last_drift_step = -10_000
        self._load_model()

    @staticmethod
    def _resolve_model_path(model_file: str) -> str:
        """Resolve the model artefact to an absolute path so it persists regardless
        of the process working directory (a relative path silently created a fresh,
        untrained model whenever the app was launched from another folder)."""
        if os.path.isabs(model_file):
            return model_file
        base_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        return os.path.join(base_dir, model_file)
    ...
    def update_online(self, features: np.ndarray, target_win: int, r_multiple: float = 1.0):
        ...
        r_multiple = SIGNED realized R-multiple of the trade (negative for a loss).

        The gradient is weighted by |R| so high-conviction outcomes move the model
        further, while the SIGN of R is applied to the learning rate rather than the
        error term. This keeps the update a proper descent step on the cross-entropy
        loss for both winners and losers (a naive `error * r_multiple` would flip the
        gradient sign on every losing trade and push the model the wrong way).
        """
        with self._lock:
            feat = np.asarray(features, dtype=float)
            if len(feat) < self.n_features:
            ...
            # ── Brier score tracking (drift detection) ────────────────────────
            self._brier_window.append(error ** 2)
            current_brier = self.get_brier_score()

            # Drift protection must NOT destroy learned signal.
            #
            # The previous implementation multiplied every weight by 0.70 whenever the
            # rolling Brier score exceeded 0.28. Two defects made that harmful:
            #   1. 0.28 is barely above the 0.25 of a coin-flip predictor, so ordinary
            #      losing streaks (which are pure variance) triggered it.
            #   2. It could fire on consecutive steps, compounding to ~50% signal loss.
            # It also reset `training_steps` to 10, restoring a large learning rate.
            #
            # Correct behaviour: treat drift as a reason to slow down and re-explore,
            # not to erase. We anneal the learning rate and let the online updates
            # re-fit. Only a sustained, severe degradation triggers a partial shrink,
            # and that shrink is rate-limited so it can never compound within a window.
            self._drift_events = getattr(self, "_drift_events", 0)
            self._last_drift_step = getattr(self, "_last_drift_step", -10_000)
            severe_brier = self.brier_drift_threshold + self.brier_drift_hysteresis

            if (
                current_brier > severe_brier
                and len(self._brier_window) >= self.brier_window_min
                and (self.training_steps - self._last_drift_step) >= self.drift_cooldown_steps
            ):
                # Gentle, rate-limited decay + cautious re-exploration. Preserves the
                # bulk of the learned signal instead of wiping it.
                self._drift_events += 1
                self.weights = np.clip(self.weights * self.drift_shrink_factor, -3.0, 3.0)
                self._last_drift_step = self.training_steps
                logger.warning(
                    f"⚠️ MODEL DRIFT: Brier={current_brier:.3f} > {severe_brier:.2f}. "
                    f"Applying rate-limited weight decay x{self.drift_shrink_factor} "
                    f"(event #{self._drift_events}, non-compounding)."
                )

            # Return-weighted learning step: scale step SIZE by |R|, preserve gradient direction
            ...
            if len(self._grad_buffer) >= self._batch_size:
                ...
                # Adaptive learning rate decay (never reset upward by drift handling)
