...
| # | Finding | I | E | Note |
|---|---|---|---|---|
| AI5 | Self-learning averages the outcome, not the forecast | H | M | ✅ **FIXED** 2026-09-20, **historical rows repaired 2026-09-22** — the forecast is no longer overwritten (112/112 rows had `expected_value == realized_pnl`; 84 had a fabricated `ai_score=85.0`); the engine now reads `realized_pnl` and `expected_value` explicitly. `tools/repair_forecast_column.py --apply` ran against the production DB: fabricated `ai_score=85.0` **84 → 0**, `expected_value == realized_pnl` **111 → 0**, NULL **0 → 112** (84 rows nulled on both fields, 28 further overwritten forecasts nulled). The forecast itself is **unrecoverable** — overwritten in place — so NULL is the honest value: calibration can skip a NULL, it will silently learn from a copied outcome. Backup at `data/jarvis_history.db.bak-20260922-121357`. **AI5 is complete.** |
...
| C8 | Gross vs net P&L; no fee columns anywhere | H | M | ✅ **FIXED** 2026-09-22 — `database.py` wrote `pnl = float(exit_deal.profit)` (gross) while `state_synchronizer.py` reported net; the two disagreed by the whole cost of the round trip. `log_trade` now sums **exit profit + entry commission + entry swap + exit commission + exit swap** (`database.py:516`), so the journal and the live strip agree. **The historical rows are still gross** — the repair needs a per-deal fee backfill from broker history, which is a separate job; only rows closed *after* this change carry the net figure |
...
| A1 | Two radar producers, two contracts, three sort keys | H | M | ✅ **FIXED** 2026-09-22 — `orchestrator.py` and `server.py` each defined their own nested 5-tuple `_radar_sort_key`, so the UI could rank a different "best" than the engine that trades. Both now import one `radar_sort_key` from `jarvis/application/radar_sort.py` (server ×2 call sites, orchestrator ×1); verified no `_radar_sort_key` definition remains anywhere |
| A2 | Parallel scan is a no-op — two process-wide MT5 locks | H | M | ✅ **FIXED** 2026-09-22 — `SignalScanner` took a `parallel_analysts: bool = False` that no production caller ever set, so production always ran the serial cluster while 39 tasks fanned out and serialized (measured 2.16s cold sweep). The flag is gone; the scanner now builds `ParallelAnalystCluster()` unconditionally. `ParallelAnalystCluster` defaults to `parallel=True`, so the 8-worker `ThreadPoolExecutor` now runs in production. **Caveat:** this makes the *analyst* fan-out parallel; the two process-wide MT5 locks are a separate, still-open constraint on broker access |
| D1 | Paper and live fills share one table, no discriminator | H | S | ✅ **FIXED** — `execution_mode` is a separate column (`live`/`paper`/`demo`/`unknown`) beside `origin`, populated at every write site and filterable on read. They answer different questions and neither derives from the other: `origin='synthetic'` covers *both* a paper fill with no quote and a live fill whose client lost the terminal, so the one column that existed silently stood in for the one the finding asked about. **The backfill is `unknown`, deliberately** — nothing in a historical row carries the mode, and guessing `live` would file unverifiable history behind the exact filter this column exists to enable. `tests/test_execution_mode_is_recorded.py` (25) |
| **A18** | **Paper mode sizes against the live broker balance** | **H** | **S** | ✅ **FIXED** 2026-09-20 — *found during implementation, not in any agent report.* `get_account_snapshot()` queried `mt5.account_info()` **before** checking `mode`, and the paper branch sat below it. That is not a rare path, it is the normal one: `init_connection()` deliberately does *not* call `mt5.initialize()` for paper (it just sets `is_connected = True`), but the **data** path does initialise a terminal, because paper trades real MT5 bars with simulated fills — so `account_info()` answers, with the live account. Measured on a paper client with a terminal reporting 762.51: `login=12345678, server=XMGlobal-MT5 5, balance=762.51` against a simulated book of 10,000. `MT5StateSynchronizer` then cached it into `state_manager.account`, so every later risk and sizing decision inherited it, and test outcomes depended on test order and on the size of a real account. The mode check now runs before any broker call; `_reconnect_if_needed()` still runs first, because it is what rewrites `mode` to `"paper"` when the package is absent. `tests/test_paper_never_sizes_against_the_broker.py` (7); **5 of 7 red** under `.scratch/mutate_a18.py`, each mutation also verified alone |
...
**Shipped 2026-09-22 — iOS responsive redesign (phone + tablet).** Phone (≤599px):
`UINavigationBar` app-bar with large title, bottom `UITabBar` with SF-Symbols-style
icons, modal bottom sheets, 44pt touch targets, rem-based Dynamic Type, safe-area
insets. Tablet (600–1024px): 236pt sidebar + split view — the same `.tt-tab` DOM as
the phone bar becomes the sidebar nav, so one state machine and no duplicated
destinations. Desktop (≥1025px) deliberately untouched. Notes in **`IOS_REDESIGN.md`**.
Verified 238/238 by `tools/verify_ui_layout.js` at 360/390/768/1440 (glass surfaces,
≥44pt tap targets, no horizontal overflow, no silently clipped content).
